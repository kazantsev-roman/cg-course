...
void CRaytraceView::AddSomeCube()
{
	...
	CSimpleDiffuseShader& shader = CreateSimpleDiffuseShader(red);
...
void CRaytraceView::AddSomeTetrahedron()
{
	...
	CSimpleDiffuseShader& shader = CreateSimpleDiffuseShader(green);

	AddTetrahedron(shader,  transform);
}
...
void CRaytraceView::AddSomeOctahedron()
{
	...
	blue.SetDiffuseColor(CVector4f(0, 0, 1, 1));
	CSimpleDiffuseShader& shader = CreateSimpleDiffuseShader(blue);

	AddOctahedron(shader, transform);
}

void CRaytraceView::AddSomeDodecahedron()
{
	CMatrix4d transform;
	transform.Translate(-3, 0.9, -2);
	transform.Rotate(-110, 0, 1, 0);

	CSimpleMaterial orange;
	orange.SetDiffuseColor(CVector4f(1, 0.65f, 0, 1));
	CSimpleDiffuseShader& shader = CreateSimpleDiffuseShader(orange);

	AddDodecahedron(shader, transform);
}

void CRaytraceView::AddSomeIcosahedron()
{
	CMatrix4d transform;
	transform.Translate(1, 2.3, -2);
	transform.Rotate(-110, 0, 1, 0);

	CSimpleMaterial pink;
	pink.SetDiffuseColor(CVector4f(1, 0.75f, 0.8f, 1));
	CSimpleDiffuseShader& shader = CreateSimpleDiffuseShader(pink);

	AddIcosahedron(shader, transform);
}
...
CSceneObject& CRaytraceView::AddTetrahedron(IShader const& shader, CMatrix4d const& transform)
{
	const auto& cube = *m_geometryObjects.emplace_back(
		std::make_unique<CTetrahedron>(transform));
